...
type AccountFleetScreenRouteProp = RouteProp<RootStackParamList, 'AccountFleetScreen'>;
type AccountFleetScreenProps = {
  route: AccountFleetScreenRouteProp;
};

const AccountFleetScreen: React.FC<AccountFleetScreenProps> = ({ route }) => {
  const { t } = useTranslation();
  ...
  const fetchVehicles = useCallback(async (page: number, refresh: boolean = false, search: string = '') => {
    ...
    try {
      const response = await api.get<PaginatedResponse>(`/client/vehicles/${responsable}/list?page=${page}&per_page=${ITEMS_PER_PAGE}&search=${search}`);
  ...
  }, [isLoading, vehicles, error, isRefreshing, handleRefresh, handleLoadMore, renderVehicleItem, renderFooter, t, fetchVehicles]);
...
export default AccountFleetScreen;
